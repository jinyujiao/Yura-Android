...
internal object TtsSecureSettings {
    ...
    fun migrateString(context: Context, legacyPrefsName: String, key: String): String {
        ...
        if (legacyValue.isNotBlank()) securePreferences.edit { putString(key, encrypt(legacyValue)) }
    ...
    private fun decrypt(value: String): String = runCatching {
        ...
        val cipher = Cipher.getInstance(TRANSFORMATION)
        cipher.init(Cipher.DECRYPT_MODE, secretKey(), GCMParameterSpec(TAG_LENGTH_BITS, payload.copyOfRange(0, IV_LENGTH_BYTES)))

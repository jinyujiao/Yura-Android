...
internal data class ReadiumCssInjector(
    ...
    private val yuraMediaIndentFix: String =
        """
        <style id="yura-readium-media-indent-fix">
        :root[style*="--USER__paraIndent"] img,
        :root[style*="--USER__paraIndent"] svg,
        :root[style*="--USER__paraIndent"] picture,
        :root[style*="--USER__paraIndent"] video,
        :root[style*="--USER__paraIndent"] canvas {
            text-indent: 0 !important;
            margin-inline-start: 0 !important;
        }
        :root[style*="--USER__paraIndent"] p:has(img),
        :root[style*="--USER__paraIndent"] p:has(svg),
        :root[style*="--USER__paraIndent"] p:has(picture),
        :root[style*="--USER__paraIndent"] div:has(> img),
        :root[style*="--USER__paraIndent"] div:has(> svg),
        :root[style*="--USER__paraIndent"] div:has(> picture),
        :root[style*="--USER__paraIndent"] figure:has(img),
        :root[style*="--USER__paraIndent"] .yura-media-block,
        :root[style*="--USER__paraIndent"] .yura-media-block * {
            text-indent: 0 !important;
        }
        :root[style*="--USER__paraIndent"] .yura-media-block {
        ...
        (function() {
            ...
            function isMediaOnly(element) {
                if (!element || hasMeaningfulText(element)) return false;
                if (!element.querySelector('img, svg, video, canvas, picture')) return false;
                return (element.innerText || '').replace(/\s+/g, '').trim().length === 0;
            }
            function markMediaBlocks() {
                ...
                for (var m = 0; m < medias.length; m++) {
                    var parent = medias[m].parentElement;
                    var depth = 0;
                    while (parent && parent !== document.body && depth < 4) {
                        var tag = parent.tagName ? parent.tagName.toLowerCase() : '';
                        if ((tag === 'p' || tag === 'div' || tag === 'figure' || tag === 'section') && isMediaOnly(parent)) {
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', markMediaBlocks);
            } else {
                markMediaBlocks();
            }
            new MutationObserver(markMediaBlocks).observe(document.documentElement, { childList: true, subtree: true });
        })();
        //]]>
        </script>
        """.trimIndent()
    ...
    return with(settings) {
        copy(
            ...
            userProperties = userProperties.copy(
                ...
                letterSpacing = letterSpacing?.let { Length.Rem(it) },

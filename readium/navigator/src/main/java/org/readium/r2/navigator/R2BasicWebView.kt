...
internal open class R2BasicWebView(context: Context, attrs: AttributeSet) : WebView(context, attrs) {
    ...
    interface Listener {
        ...
        /**
         * Called when the WebView text selection becomes active.
         */
        fun onSelectionStart(webView: R2BasicWebView) {}

        /**
         * Called when the WebView text selection is cleared.
         */
        fun onSelectionEnd(webView: R2BasicWebView) {}
    ...
    fun onSelectionStart() {
        isSelecting = true
        listener?.onSelectionStart(this)
    }

    @android.webkit.JavascriptInterface
    fun onSelectionEnd() {
        isSelecting = false
        listener?.onSelectionEnd(this)
    }
    ...
    override fun startActionMode(callback: ActionMode.Callback?): ActionMode? {
        ...
        val wrapper = CallbackWrapper(
            SelectionActionModeCallbackWrapper(
                webViewCallback = callback,
                appCallback = customCallback,
            )
        )
        return parent.startActionModeForChild(this, wrapper)
    }
    ...
    inner class SelectionActionModeCallbackWrapper(
        val webViewCallback: ActionMode.Callback?,
        val appCallback: ActionMode.Callback,
    ) : ActionMode.Callback {
        override fun onCreateActionMode(mode: ActionMode, menu: Menu): Boolean {
            val webViewCreated = webViewCallback?.onCreateActionMode(mode, menu) ?: true
            val appCreated = appCallback.onCreateActionMode(mode, menu)
            return webViewCreated && appCreated
        }

        override fun onPrepareActionMode(mode: ActionMode, menu: Menu): Boolean {
            val webViewPrepared = webViewCallback?.onPrepareActionMode(mode, menu) ?: false
            val appPrepared = appCallback.onPrepareActionMode(mode, menu)
            return webViewPrepared || appPrepared
        }

        override fun onActionItemClicked(mode: ActionMode, item: MenuItem): Boolean {
            if (appCallback.onActionItemClicked(mode, item)) {
                return true
            }
            return webViewCallback?.onActionItemClicked(mode, item) ?: false
        }

        override fun onDestroyActionMode(mode: ActionMode) {
            appCallback.onDestroyActionMode(mode)
            webViewCallback?.onDestroyActionMode(mode)
        }
    }

...
import org.readium.r2.shared.util.mediatype.MediaType
    ...
    // Fallback cover ID if not provided in metadata
    private val resolvedCoverId: String? by lazy {
        coverId ?: findFallbackCoverId()
    }

    private fun findFallbackCoverId(): String? {
        // 1. Try to find an item with "cover" in its id
        val coverById = manifest.firstOrNull { item ->
            item.id?.contains("cover", ignoreCase = true) == true &&
            item.mediaType?.startsWith("image/") == true
        }
        if (coverById != null) return coverById.id

        // 2. Try to find common cover filenames
        val coverNames = listOf("cover.jpg", "cover.png", "cover.jpeg", "cover.gif", "cover.svg")
        val coverByFilename = manifest.firstOrNull { item ->
            val hrefStr = item.href.toString().lowercase()
            coverNames.any { name -> hrefStr.endsWith(name) || hrefStr.endsWith("/$name") }
        }
        if (coverByFilename != null) return coverByFilename.id

        // 3. Try to find the first image in the manifest
        val firstImage = manifest.firstOrNull { item ->
            item.mediaType?.startsWith("image/") == true
        }
    ...
    private fun computePropertiesAndRels(item: Item, itemref: Itemref?): Pair<Set<String>, Properties> {
        ...
        if (resolvedCoverId != null && item.id == resolvedCoverId) rels.add("cover")

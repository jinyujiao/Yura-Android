...
internal class CloudTtsClient {
    fun synthesizeMimo(text: String, apiKey: String, voice: String, output: File) {
        ...
        val body = JSONObject()
            ...
            .put("messages", JSONArray()
                .put(JSONObject().put("role", "user").put("content", "请用自然、清晰、适合阅读的语气朗读。"))
        ...
        output.writeBytes(Base64.getDecoder().decode(audioBase64))
    }
    ...
    fun synthesizeMicrosoft(text: String, apiKey: String, region: String, voice: String, output: File) {
        ...
        connection.inputStream.use { input -> output.outputStream().use(input::copyTo) }
    }

    fun fetchMicrosoftVoices(region: String, apiKey: String): List<SimpleTtsController.MicrosoftVoice> {
        ...
        return List(array.length()) { index -> array.getJSONObject(index) }
            .mapNotNull { item ->
                ...
                SimpleTtsController.MicrosoftVoice(shortName, listOfNotNull(localName ?: displayName, locale).joinToString(" · "), locale)
            }
            .distinctBy { it.shortName }
            .sortedWith(compareBy<SimpleTtsController.MicrosoftVoice> { if (it.locale.startsWith("zh-", true)) 0 else 1 }.thenBy { it.locale }.thenBy { it.displayName })
    }
